...
def main(config_path: Optional[Path] = Argument(None),
    ...
    for batch_data in batch:
        # T = batch_data['data'].shape[-1]
        # n_steps = routine.n_steps or (T - 1)
        routine = routine.cuda()
        #batch_data = routine.convert_data(batch_data)
        routine.warmup()

        preds = routine.infer(batch_data)

        file_path = "/scratch/xc2695/hw2d_fourierflow/hw2d_predictions_density.h5"
        with h5py.File(file_path, 'a') as f:
            if 'predictions' not in f:
                f.create_dataset('predictions', data=preds.cpu().numpy(), maxshape=(None, *preds.shape[1:]),
                                 chunks=True)
            else:
                f['predictions'].resize((f['predictions'].shape[0] + preds.shape[0]), axis=0)
                f['predictions'][-preds.shape[0]:] = preds.cpu().numpy()

    # elapsed = time.time() - start
    # elapsed /= len(batch_data['data'])
