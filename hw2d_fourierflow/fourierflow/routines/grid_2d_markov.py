...
class Grid2DMarkovExperiment(Routine):
    def __init__(self,
                 ...
                 use_fourier_position: bool = False, # not using fourier encoding by default
                 noise_std: float = 0.0,
    ...
    def forward(self, data):
        _, _, preds, _=self._valid_step(data)
        return preds
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.pred_path:
            ...
            for b in range(B):
                ...
                for t in range(T):
                    wb.append(preds[b, ..., t])
                ws.append(np.stack(wb, axis=-1))

            ws = np.stack(ws, axis=0)

            dim_names = ('sample', 'x', 'y', 'time')
            data_vars = {'preds': (dim_names, ws)}
            ds = xr.Dataset(
                data_vars=data_vars,
                coords={
                    'sample': range(B),
